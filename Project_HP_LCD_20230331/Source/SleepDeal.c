...
void InitWakeUp_Base(void)
{
	...
	// DI1#define MCUI_ENI_DI1		(PORT_IN_GPIOC->bit13)
...
void IOstatus_Base(void)
{
	...
	//__delay_ms(1000);
	// RCC_AHBPeriphClockCmd(RCC_AHBPeriph_GPIOA, DISABLE); // 开启GPIOA的外设时钟
	// RCC_AHBPeriphClockCmd(RCC_AHBPeriph_GPIOB, DISABLE); // 开启GPIOB的外设时钟
	// RCC_AHBPeriphClockCmd(RCC_AHBPeriph_GPIOC, DISABLE); // 开启GPIOC的外设时钟
}
...
void SleepDeal_Normal_Select(void)
{
	...
	{ // 核心
		Sleep_Mode.bits.b1NormalSleep_L1 = 0;
		Sleep_Mode.bits.b1NormalSleep_L2 = 0; // Sleep_Mode用于外部控制进入休眠标志位
		Sleep_Mode.bits.b1NormalSleep_L3 = 0;
		Sleep_Status = SLEEP_HICCUP_SHIFT;
		return;
	}
...
void SleepDeal_Shift(void)
{
	...
	{
		Sleep_Status = SLEEP_HICCUP_TEST; // 12
	}
...
{ // 检测
	...
	if (!Sleep_Mode.bits.b1TestSleep)
	{									   // 加强雍余设计
		Sleep_Status = SLEEP_HICCUP_SHIFT; // 0
		return;
	}
	...
	{ // 10s——Test
		s_u16HaltTestCnt = 0;
		Sleep_Status = SLEEP_HICCUP_CONTINUE; // 1
	}
}
...
void WakeUp_Judeg(void)
{
	//		while (gu8_SOC_DI)
	//		{ // 别的唤醒就为0了，不会进入
	//			// LedBar_Init();			//初始化DI开关，感觉应该不用，原来就有中断在
	//			delay_1ms(1000);
	//			gu8_SOC_DI = 0; // 一定要清0，不然进入休眠后，别的唤醒就一直不能清0，卡死在这里
	//			if (MCUI_SOC_KEY == 0)
	//			{
	//				break;
	//			}
	...
	//				// 继续进入休眠
	//				Sys_StopMode();
	//				// 唤醒继续从这里执行程序，如果是SOC-DI唤醒，继续执行这个循环，如果是别的唤醒，则跳过
	//			}
	//		}
}
...
{ // 休眠功能要开启
	...
	// { // 开机完毕再进入	//BMS第一次开机标志位，初始为1，确定能打开管子视为系统初始化完毕
	// 	return;
	// }
	// else
	// {
	// 	SystemStatus.bits.b1Status_ToSleep = 1; // 进入休眠命令状态
	// }

	// if (Sleep_Mode.bits.b1_ToSleepFlag)
	// {									   // 当为1时，1s后进入休眠，别的时间为0
	// 	LogRecord_Flag.bits.Log_Sleep = 1; // 日志休眠
	// 	return;
	// }
	...
	// {			// b1ForceToSleep_L2:1;//外部操控进入第二级休眠
	// 	return; // 如果是强制进入休眠的则必须快点进入休眠，不能拖		b1ForceToSleep_L3 :1;//外部操控进入第三级休眠
	// }

	switch (Sleep_Status)
	{
	case SLEEP_HICCUP_SHIFT: // 0		//先跳到这里，再跳到SleepDeal_Continue()，然后进入别的循环
		SleepDeal_Shift();	 // 进行跳转			//主控跳转函数，开机执行一遍没事进入核心循环函数
		break;
	case SLEEP_HICCUP_NORMAL_SELECT: // 8
		// SleepDeal_Normal_Select();
		break;
	case SLEEP_HICCUP_TEST: // 12
		SleepDeal_Test();	// 检测
		break;
		// case SLEEP_HICCUP_OVERCUR: // 2
		// 	SleepDeal_OverCurrent();
		// 	break;
		// case SLEEP_HICCUP_OVDELTA: // 3
...
{
	MCU_RESET(); // 确保所有未完成的内存访问（包括缓冲写入）均已完成
}
