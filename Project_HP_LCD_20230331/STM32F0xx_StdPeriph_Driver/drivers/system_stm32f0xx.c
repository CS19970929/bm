...
static void SetSysClock(void)
{
  ...
  if (HSEStatus == (uint32_t)0x01)
  {
    ...
    RCC->CFGR |= (uint32_t)(RCC_CFGR_PLLSRC_PREDIV1 | RCC_CFGR_PLLXTPRE_PREDIV1 | RCC_CFGR_PLLMULL4);	//改为4倍频
  ...
  { /* If HSE fails to start-up, the application will have wrong clock 
		...
		RCC_HCLKConfig(RCC_SYSCLK_Div1);	  					//AHP不分频(看框图)
		RCC_PCLKConfig(RCC_HCLK_Div1);		 					//APB clock = HCLK 不分频

		//设置 PLL 时钟源及倍频系数
		//RCC_PLLCmd(DISABLE);
		RCC_PLLConfig(RCC_PLLSource_HSI_Div2, RCC_PLLMul_12);	//HSI/2 selected as PLL input clock(看框图)
		RCC_PLLCmd(ENABLE);										//如果PLL被用于系统时钟,那么它不能被使能？？？？？
		...
		while(RCC_GetSYSCLKSource() != RCC_CFGR_SWS_PLL);		//需与被选择的系统时钟对应起来，RCC_SYSCLKSource_PLL
																//等待PLL成功用作于系统时钟的时钟源
		//MCO 引脚初始化
